...
- Confirm `/`, `/international/`, `/media/`, `/contact/`, `/ja/`,
  `/ja/international/`, `/ja/media/`, and `/ja/contact/` return successful
  responses.
- Confirm `/consulting/` redirects to `/contact/` and `/ja/consulting/`
  redirects to `/ja/contact/`.
...
- Open all eight direct Markdown page mirrors linked by `/llms.txt`, including
  `/international.md` and its Japanese equivalent. Confirm a `text/markdown`
  content type, `X-Robots-Tag: noindex, follow`, canonical HTML links in the
  documents, and factual agreement with the HTML pages.
- Confirm the eight proposal-style `index.html.md` compatibility aliases
