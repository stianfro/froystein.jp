...
container image and must be enabled by the deployment at runtime. This lets the
lab deployment connect to Umami while production remains unchanged until its
...
source and have a 14-day retention period. These settings are managed in the
lab repository rather than this image.
